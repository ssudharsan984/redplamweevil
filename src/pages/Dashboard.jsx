...
export default function Dashboard() {
  ...
  useEffect(() => {
    const unsub = subscribeToRecentTraps(data => { setTraps(data); setLoading(false) }, 6)
    return unsub
  }, [])
  ...
  return (
    <Layout>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-6">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl bg-primary-600 flex items-center justify-center text-xl font-extrabold text-white flex-shrink-0 shadow-sm">
              {initials}
            </div>
            <div>
              ...
              <div className="flex items-center gap-2.5 mt-1.5 flex-wrap">
                <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-primary-50 text-primary-700 text-xs font-semibold rounded-full border border-primary-100">
                  <div className="w-3 h-3"><ShieldIcon /></div>{role}
                </span>
                {phone && (
                  <span className="inline-flex items-center gap-1 text-xs text-gray-500">
                    <div className="w-3 h-3 text-gray-400"><PhoneIcon /></div>{phone}
                  </span>
                )}
                {lastUpdated && <span className="text-xs text-gray-400">Updated {formatTimeAgo(lastUpdated)}</span>}
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Link to="/profile" className="btn-secondary text-sm py-2 px-3.5 flex items-center gap-1.5">
              <div className="w-3.5 h-3.5"><EditIcon /></div>Edit Profile
            </Link>
            <button onClick={handleLogout} className="btn-danger text-sm py-2 px-3.5 flex items-center gap-1.5">
              <div className="w-3.5 h-3.5"><LogoutIcon /></div>Sign Out
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard label="Active Traps" value={loading ? '—' : activeTraps}   icon={<SignalIcon />}  bgClass="bg-violet-50"  colorClass="text-violet-600" />
        <StatCard label="RPW Alerts"   value={loading ? '—' : totalDetected} icon={<AlertIcon />}  bgClass="bg-danger-50"  colorClass="text-danger-600" />
        <StatCard label="Clear Scans"  value={loading ? '—' : clearScans}    icon={<CheckIcon />}  bgClass="bg-primary-50" colorClass="text-primary-600" />
      </div>

      {/* Alert banner */}
      {!loading && totalDetected > 0 && (
        <div className="mb-6 p-4 bg-danger-50 border border-danger-200 rounded-2xl flex items-start gap-3">
          <div className="w-5 h-5 text-danger-500 flex-shrink-0 mt-0.5"><AlertIcon /></div>
          <div>
            <p className="font-semibold text-danger-800 text-sm">Red Palm Weevil Detected</p>
            <p className="text-xs text-danger-600 mt-0.5">
              {totalDetected} detection{totalDetected > 1 ? 's' : ''} found. Check Trap Details for images and full information.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5 bg-primary-50 border border-primary-200 px-3 py-1.5 rounded-full">
          <span className="pulse-dot" />
          <span className="text-xs text-primary-700 font-semibold">Live</span>
        </div>
      </div>

      {loading ? <LoadingSpinner message="Fetching detections..." />
        : traps.length === 0 ? <EmptyState title="No detections yet" message="Detection results from your YOLO script will appear here in real-time." />
        : (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {traps.map(trap => <DetectionCard key={trap.id} detection={trap} />)}
          </div>
        )}
      ...
      <div className="mt-8">
        <h2 className="section-title mb-4 flex items-center gap-2">
          <div className="w-4 h-4 text-gray-500"><BellIcon /></div>Notifications
        </h2>
        {totalDetected > 0 ? (
          <div className="space-y-2.5">
            {traps.filter(isDetected).slice(0, 3).map(trap => (
              <div key={trap.id} className="bg-white rounded-xl border border-danger-100 p-4 flex items-center gap-3 shadow-sm">
                <div className="w-8 h-8 bg-danger-50 rounded-xl flex items-center justify-center flex-shrink-0">
                  <div className="w-4 h-4 text-danger-500"><AlertIcon /></div>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-gray-900 text-sm">RPW Detected — {trap.trapId}</p>
                  <p className="text-xs text-gray-500 mt-0.5">{trap.location || 'Unknown location'} · {formatTimeAgo(trap.timestamp)}</p>
                </div>
                {trap.confidence != null && (
                  <span className="text-xs font-bold text-danger-600 bg-danger-50 px-2 py-1 rounded-lg flex-shrink-0">{trap.confidence}%</span>
                )}
              </div>
            ))}
          </div>
        ) : (
          <div className="bg-white rounded-xl border border-primary-100 p-4 flex items-center gap-3 shadow-sm">
            <div className="w-8 h-8 bg-primary-50 rounded-xl flex items-center justify-center flex-shrink-0">
              <div className="w-4 h-4 text-primary-500"><CheckIcon /></div>
            </div>
            <p className="text-sm text-gray-600">All clear — no RPW detected in recent scans.</p>
          </div>
        )}
      </div>
    </Layout>
